...
function handleScroll() {
  ...
  if (
    position >= $('#services').position().top &&
    position < $('#products').position().top
  ) {
    $('.nav-link[href="#services"]').addClass('active');
    var relativePos = position - $('#services').position().top;
    if (
      relativePos >= $('#exporter').position().top &&
      relativePos < $('#projects').position().top
    ) {
